...
function buttonEncriptar(){
    if (validarTexto(campoTexto.value) === true){
        ...
        campoRespuesta.innerText = texto; // Usar innerText porque un "p" no tiene atributio value
        copiar.style.display = "block"; // Aparece el boton de copiar que está oculto
...
function desencriptar (fraseDencriptada){
    for (let i = 0; i < matriz_code.length; i++){
        if (fraseDencriptada.includes(matriz_code[i][0])){
